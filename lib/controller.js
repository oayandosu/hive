...
hive.app.use(express.staticProvider(hive.path + '/public'));
...
function view(route, context) {
	...
	// look for the view in hive.views
	var hasView = false;
	if(hive.views) {
		hasView = !!(hive.views[context] && hive.views[context][routeViewName]);
	}
	
	// return false or if the view was found, return the route
	return hasView && (context + '/' + routeViewName);
